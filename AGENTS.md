...
- `questions.js`: Live question bank, answer-choice definitions, condition labels, selectable-module definitions, and pure focused-routing functions exported as `window.SCREENING_QUESTION_DATA`.
- `scoring.js`: Pure scoring core (no DOM). Weight vectors (`WEIGHTS`), discriminator caps, condition scorers, validity flags, threshold labels, overlap-only findings, assessed-scope normalization, and `buildContext`/`buildReport`. Loaded as a classic `<script>` before `script.js` and also `module.exports`ed for `tests.js` in Node.
- `script.js`: Grouped scoped-question display, form reading, active required-answer validation/progress, scope-aware localStorage persistence, HTML report rendering, PDF generation, print handling, and initialization. `scoreAssessment()` passes only active questions to `buildReport()` in `scoring.js`. Holds the shared `CLINICAL_SOURCES` array that renders both source lists.
...
- `tests.js`: Dependency-free regression suite for scoring, focused routing, and PDF helpers, run with `node tests.js`. Asserts weight sums, discriminator caps, validity boundaries, focused-module counts/semantics, and a golden comprehensive-report baseline.
...
- Keep questions grouped in coherent source sections and routed through the selectable ADHD, autism, OCD, CDS, and anxiety modules. Focused routing must keep every item in a selected condition's scored module; only explicitly mapped bridge questions may come from an unselected condition, and they must never produce that condition's percentage.
- Keep all **active required** questions required for generate, export, and print. Hidden modules do not gate, enter progress, or reach scoring, although their saved answers may be retained for reselection. The only optional section remains strengths: show it only for ADHD/autism scopes, exclude it from progress/completion, and feed no condition percentage. If optional items change, do **not** bump `STORAGE_VERSION`; `meta.questionCount` tracks the active required count.
- Keep current-safety and priority mania/psychosis prompts in every scope. The broader overlap/rule-out layer may be turned off, but the report must record that limitation.
- Unselected conditions must be represented as "not assessed," never as 0% or a low screen. AuDHD may be calculated only when both ADHD and autism modules are selected.
