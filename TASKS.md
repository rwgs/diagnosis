...
10. **Modular focused screening** — selectable ADHD, autism, OCD, CDS, and anxiety modules with grouped presentation, routed overlap/differential questions, and explicit not-assessed report states. **Done** on `feature/modular-screening`.
...
| 10. Modular focused screening | **Done** (`feature/modular-screening`; comprehensive default plus focused scopes from 32–180 required questions with recommended overlap checks) |
...
## 8. Fourth review pass (2026-07-11) — DONE (implemented 2026-07-12)

Full static review of `index.html`, `styles.css`, `questions.js`, `scoring.js`, `script.js`, `tests.js`, `README.md`, `QUESTIONS.md`, and this backlog. All Tier 1–3 findings below were implemented on 2026-07-12, except two items that are design/tooling decisions rather than code changes (Tier 1.4 external validation and Tier 2.1 automated headless harness), which are recorded as remaining work. No new questions were added; `STORAGE_VERSION` is unchanged (the PTSD change was a display-label rename only, keyed on the unchanged `ptsdComplex` domain). A new pure module `pdf.js` was added for the testable PDF layout helpers.

**Current validation baseline (2026-07-12):** `node --check` clean on `questions.js`, `scoring.js`, `pdf.js`, `script.js`; `node tests.js` reports **2138 passed, 0 failed** (adds §7 PDF layout helpers and §8 bank-structure invariants to the prior scoring/report coverage); bank unchanged at **225 items (218 required + 7 optional strengths)**. This is the single current baseline; the point-in-time counts inside older change records (2004, 2036, 2056, 2065, 2071, 2081) are left as-is because they describe the suite at that change.

### Tier 1 — Clinical safety, adult-pathway enforcement, and framing — DONE

1. **Safety “Unsure” now reads as uncertainty, not endorsement.** `scoreDifferential()` classifies each risk item by its stored answer label into `endorsed` (Yes), `uncertain` (Unsure), `declined` (Prefer not to say), or `none` via `riskState()`. The `safety` object is `{ percent, endorsed, uncertain, declined, note }`; the flag list renders `(unsure)` and `(declined to answer)` with no pseudo-severity percentage, and only a genuine "Yes" shows a percent. The conservative ≥50% flag still fires for all three. The state-specific note is separate from a standing immediate-danger line (`SAFETY_IMMEDIATE_DANGER`) that the HTML and PDF safety blocks always render, so a report read without the intro still carries crisis guidance. `tests.js` §4f was rewritten to cover Yes / Unsure / No / Prefer-not-to-say and their combinations.

2. **Adult age boundary enforced on all report actions.** `validateAge()`/`clearAgeError()` in `script.js`: age stays optional, but a supplied value must be a whole number 18–120 (`/^\d+$/` plus range). Invalid values block generate/export/print, show `#ageError` (wired via `aria-describedby`, with `aria-invalid` on the field), and move focus to the field; the error clears live once corrected and on reset. `index.html` adds `step="1"`, `#ageHelp`, and `#ageError`. DOM-flow coverage is in the README manual browser QA matrix (see Tier 2.1).

3. **PTSD / complex-PTSD claim corrected (minimal rename).** The display label is now "PTSD / trauma-related pattern" in `scoreDifferential`/`buildRecommendations`; the recommendation frames the dissociation item as an associated prompt and points to ICD-11 complex-PTSD's disturbances-in-self-organization (affect regulation, self-concept, relationships) rather than implying a complex-PTSD screen. `README.md`'s "five-cluster" wording was corrected. The internal `ptsdComplex` domain key is unchanged (no saved-answer impact). **Deferred option:** building original DSO coverage to legitimately retain a complex-PTSD label — out of scope for the minimal fix and not pursued.

4. **Unvalidated-score framing added; external validation still owed.** The HTML report header and the PDF now carry a prominent "How to read these numbers" callout: the percentages and low/moderate/high bands are unvalidated heuristic construct-match indices, uncalibrated, with no established sensitivity/specificity, and a single global-impairment answer plus a cross-condition trait-stability composite feed several conditions. `README.md`'s Clinical Framing gained a bold unvalidated-heuristic bullet. **Not a code task (remains open):** obtaining clinical/psychometric review and testing against appropriate reference data before any "accuracy" claim; weights were deliberately not tuned to the golden fixture. See remaining work below.

### Tier 2 — Browser, accessibility, storage, and report robustness — DONE (harness deferred)

1. **Browser-path coverage — documented manual matrix; automated harness deferred.** A manual browser QA matrix in `README.md` covers initialization, deterministic mixed rendering, restore states, blocked/full storage, missing-answer advance, age validation, reset, generate/HTML-escaping, PDF export, print visibility, reduced motion, and keyboard/screen-reader. A genuinely automated harness needs a browser driver or DOM shim (Playwright/jsdom), which conflicts with the dependency-free/no-build constraint, so it is deferred pending approval to add dev-only tooling. Where logic could be pulled out of the DOM it was: the PDF layout helpers moved to `pdf.js` and are unit-tested (Tier 2.4 / §7).

2. **Reduced motion respected.** `@media (prefers-reduced-motion: reduce)` in `styles.css` disables smooth scrolling and effectively disables transitions/animations; `script.js` adds `prefersReducedMotion()`/`scrollBehavior()` so the two `scrollIntoView` calls jump instantly under that preference.

3. **Persistence failures made accurate and non-destructive.** `storageSetItem` distinguishes a quota error (`isQuotaError`, returns `"quota"`, storage stays available) from blocked storage (returns `false`, flips `storageAvailable`); `saveAnswers` shows distinct saved / full / blocked messages. A corrupt/unparseable payload is now reported (`STORAGE_CORRUPT_MESSAGE`) and left in place rather than silently deleted, with the session continuing in memory until the next successful save. `cancelPendingSave()` cancels the debounced text-save on reset so a keystroke just before "Clear Answers" cannot recreate the cleared payload.

4. **PDF layout helpers hardened and tested.** `wrapPdfText` (moved to `pdf.js`) hard-splits any token longer than `maxChars`, so a long URL or pasted token cannot overflow the printable width. `paginatePdfLines` moved alongside it. `tests.js` §7 covers hard-split (characters preserved), normal word wrap, empty text, multi-page pagination with no line below the margin, and `keepWithNext` orphan-prevention. Source-list URLs are now included in the PDF (via the shared `CLINICAL_SOURCES`), relying on this wrap. The ASCII/transliteration limit stays documented in `README.md`.

5. **Theme init browser-compatible.** `initTheme()` uses `MediaQueryList.addEventListener` when present and falls back to `addListener` (older Safari), so a missing modern API cannot throw before the questionnaire and button listeners are wired.

### Tier 3 — Documentation, sources, and invariant maintenance — DONE

1. **Sources centralized and expanded.** One `CLINICAL_SOURCES` array in `script.js` renders both the on-screen `#sourcesList` and the PDF source bullets (with URLs), so their names and coverage cannot drift; `SOURCES_FRAMING_NOTE` + `SOURCES_AUDIT_DATE` (2026-07-11) state the sources inform construct coverage/framing only, not this app's items, scoring, or bands. Added the VA National Center for PTSD (DSM-5, complex-PTSD assessment), NIMH borderline personality disorder, the International OCD Foundation on OCD-related disorders (hoarding, body dysmorphic disorder), and the 988 Suicide & Crisis Lifeline; all five new URLs were fetched and confirmed live during the change.

2. **Bank-structure invariants encoded.** `tests.js` §8 asserts unique question ids, supported types (`scale`/`choice`), non-empty `condition`/`domain` on every item, conditions ⊆ a known set, choice questions referencing a defined choice set (and scale questions carrying none), unique numeric values within each `CHOICES` set and `SCALE`, exact 225/218/7 counts, optional==strengths, that each of the 16 report-referenced differential domains exists in the bank, and that each core scored condition has ≥1 item.

3. **`QUESTIONS.md` reconciled.** The stale DIVA-5 intro was corrected; a top-of-file status summary plus a Status column in the Coverage summary now mark each candidate Implemented / Unimplemented / Retired, verified against `questions.js` on 2026-07-12. Only `diva-rel2`, `caars-self1`, and `caars-rel2` remain genuinely open candidates; the "priority additions" list is now a record of completed work.

4. **One current validation baseline.** Established at the top of this section (2138 assertions, 2026-07-12). Historical counts inside earlier change records are preserved as point-in-time state.

### Remaining work from this review (design/tooling, not code)

- **Clinical/psychometric validation (from Tier 1.4).** The scores and bands are labelled as unvalidated heuristics, but no calibration study, criterion sample, or sensitivity/specificity evidence exists. Before any "accuracy" claim, obtain clinical/psychometric review, test against appropriate reference data, and decide whether the shared global-impairment answer and trait-stability composite need condition-specific prompts or score-neutral sensitivity displays. Do not tune weights to the current golden fixture.
- **Automated browser harness (from Tier 2.1).** The manual QA matrix is the interim. An automated harness (Playwright or jsdom) is deferred because it adds dev-only tooling that conflicts with the dependency-free/no-build constraint; revisit if that constraint is relaxed.
- **Optional deeper PTSD path (from Tier 1.3).** Only if the project wants to retain a genuine complex-PTSD label: build original disturbances-in-self-organization coverage, a trauma-exposure/duration interpretation, scoring boundaries, and tests — without simply growing the required bank.

---

## Suggested implementation order for remaining work

All Section 8 code findings, the Section 9 standalone-question audit, and the Section 10 modular focused-screen branch are implemented. What remains is external validation, review/merge of the feature branch, and an optional tooling decision:

1. **Review `feature/modular-screening` in a browser** — exercise comprehensive and focused scopes, confirm mobile/keyboard/print/PDF behavior, then decide whether to merge it into `main`.
2. **Clinical/psychometric validation** of the scores and bands before any "accuracy" claim, and a decision on whether the shared global-impairment answer and trait-stability composite need condition-specific prompts or score-neutral sensitivity displays. Do not tune weights to the golden fixture.
3. **Automated browser harness** (Playwright or jsdom) — only if the dependency-free/no-build constraint is relaxed; otherwise the README manual QA matrix stands.
4. **Optional deeper complex-PTSD coverage** — only if the project wants a genuine complex-PTSD label (build original DSO coverage, exposure/duration interpretation, boundaries, and tests).

The required bank remains capped at **218 items plus 7 optional strengths**. New questions are not the next step: the remaining work is validation and delivery reliability, not more items.
...
## 10. Modular focused screening (2026-08-31) — DONE
