...
Answers are stored only in the browser's local storage on the current device. On a shared or public computer, other people using the same browser could see or restore them, so the intro advises clearing answers when finished. If the browser blocks storage access (for example private mode, or a cookies/site-data setting), or storage is full, the app degrades gracefully: it reports the specific reason answers cannot be saved this session, and generating, exporting, and printing a report still work in-memory.

## Current Questionnaire

- Comprehensive mode has 218 required questions, plus an optional, unscored strengths section of 7 items (225 items in total). It remains the default for compatibility with existing saved answers and the locked scoring baseline.
- Adult-focused wording.
- A scope selector can administer any combination of the complete ADHD, autism, OCD, CDS, and anxiety modules. Selecting ADHD and autism together also calculates the AuDHD co-occurrence output.
- Questions are grouped into their coherent source sections. Focused routing keeps complete selected condition modules, the context/validity/discriminator items their formulas need, four always-present safety/priority questions, and—when enabled—a mapped overlap/rule-out layer.
- With recommended overlap checks enabled, single-module required totals are ADHD 95, autism 116, OCD 41, CDS 32, and anxiety 32; ADHD + autism is 180. ADHD without the broader overlap layer is 75. Totals include shared questions and therefore are not additive.
- A small number of anxiety or OCD questions can appear as explicitly labelled overlap-only prompts when those full modules are unselected. Elevated bridge answers produce discussion prompts only: they never calculate a partial anxiety/OCD percentage or imply a positive or negative screen.
...
- All visible required questions in the active scope must be answered before a report can be generated, exported, or printed. Hidden modules do not gate or enter progress. Their prior answers are retained locally if the module is reselected, but they are excluded from scoring. The optional strengths section appears only for ADHD/autism scopes, can be completed or skipped freely, and never gates the report.
- Current-safety and priority mania/psychosis prompts remain in every scope. The broader overlap/rule-out layer is recommended but can be omitted; the report records that limitation.
...
- Differential flags: sleep/circadian disruption, sleep apnea/daytime sleepiness, mood/depression, burnout, trauma/stress/dissociation, PTSD / trauma-related pattern (the four DSM-5 PTSD symptom clusters — intrusion, avoidance, negative cognition/mood, arousal/reactivity — plus an associated dissociation/derealisation prompt; this is a trauma-related screen, not a validated complex-PTSD measure, and ICD-11 complex PTSD would additionally require disturbances in affect regulation, self-concept, and relationships), borderline / emotional dysregulation (identity instability, idealisation–devaluation, chronic emptiness, fear of abandonment), substance/medication effects, medical factors, mania/hypomania, psychosis-like experiences, learning/language/coordination history, self-harm risk, and harm-to-others risk.
...
- Screening-match percentages only for the selected modules. Unselected ADHD, autism, OCD, CDS, or anxiety modules are explicitly listed as "Not assessed" and never displayed as 0%. AuDHD appears only when both ADHD and autism are selected.
- A prominent scope statement in the HTML, PDF, and print report records selected modules, unassessed modules, whether AuDHD was calculated, and whether broader overlap/rule-out checks were included.
...
- Overlap-only discussion prompts, when an unselected-module bridge group is elevated. These are plainly separated from condition scores and recommend the full module or clinical discussion rather than assigning a percentage.
...
- The scope selector uses native checkboxes in a labelled group, reports the active required count in a small polite live region, and moves focus to the first module checkbox if report generation is attempted with no module selected.
...
- A reduced-motion preference (`prefers-reduced-motion: reduce`) is respected: smooth scrolling and UI transitions/animations are disabled in CSS, and the JavaScript focus/error scrolling jumps instantly instead of animating.
- The theme layer follows later OS colour-scheme changes using the modern `MediaQueryList.addEventListener`, falling back to the legacy `addListener` on older engines (such as older Safari) so initialisation never throws before the questionnaire and button listeners are wired.
- If an age is supplied it must be a whole number from 18 to 120; an invalid value shows an inline error associated with the field via `aria-describedby`/`aria-invalid` and moves focus to it before any report is generated, exported, or printed.
- The first missing active question is highlighted and focused when report generation is attempted too early; answering each highlighted missing question advances to the next visible question in grouped on-screen order. Hidden modules and optional strengths never enter this flow.
...
- Saved answers and the selected scope persist per-device in local storage under a schema version and the active required count. Hidden-module and optional-strength answers can still be retained, but they do not affect focused progress or scoring. A restore distinguishes three cases: saved answers that no longer map to the current questionnaire are reported as a partial restore ("Restored N of M …"); a version change or a grown active question set with all saved answers still intact is reported as "review and answer any remaining questions" without falsely claiming data loss; and an unchanged scope/bank restores quietly. Legacy saves without scope default to comprehensive mode. Persistence failures are distinguished rather than collapsed into one message: blocked storage reports that answers cannot be saved this session; a full-storage quota error reports that storage is full; and an unparseable/corrupt payload is reported and left in place. Clearing answers cancels any pending debounced text save first and restores the default comprehensive scope.
...
- `questions.js` contains the live question bank, answer-choice definitions, condition labels, selectable-module definitions, and pure question-routing functions. Its scope maps cover context, validity, discriminator, differential, safety/priority, overlap-only bridge, and optional-strength roles.
- `scoring.js` contains the pure scoring core: weight vectors (`WEIGHTS`), condition scorers, discriminator and validity logic, threshold labels, overlap-only finding generation, scope normalization, the clinical-discussion-point generator (`buildRecommendations`), the optional-strengths list builder (`buildStrengths`), and the `buildContext`/`buildReport` entry points. `buildReport` calculates only selected conditions and adds AuDHD only when both required modules are present.
- `script.js` contains grouped scoped-question display, form reading, dynamic required-answer validation/progress, scope-aware persistence, HTML/PDF rendering, age validation, and event handlers. Its `scoreAssessment()` passes only active questions to `buildReport()`. It also holds the shared `CLINICAL_SOURCES` array that renders both source lists.
...
- `tests.js` is a dependency-free regression suite for `scoring.js`, routing, and `pdf.js`, run with `node tests.js`. Alongside the scoring/PDF invariants and locked comprehensive golden baseline, it asserts focused required counts, complete selected modules, safety/priority inclusion, overlap-off behavior, explicit unassessed states, AuDHD gating, and the rule that bridge answers can create discussion prompts but not unselected condition scores.
...
- **Scope and grouped rendering:** comprehensive mode shows 218 required questions in source-section order; each focused preset/count matches the documented routing totals; changing scope hides/shows whole routed rows without losing in-session answers; question numbers remain contiguous; bridge-only sections are labelled; and optional strengths appear only for ADHD/autism.
- **Not-assessed semantics:** generate focused reports for every single module plus ADHD + autism; confirm only selected scores appear, AuDHD appears only for ADHD + autism, excluded modules are named as not assessed, bridge prompts carry no percentage, and HTML/PDF/print all state the scope.
